...
alertcondition(longCondition, title="Strategy Long Entry Alert", message="OpenStock Buy Signal on {{ticker}} at {{close}} [${normType}]")
alertcondition(exitCondition, title="Strategy Long Exit Alert", message="OpenStock Sell/Exit Signal on {{ticker}} at {{close}} [${normType}]")
`;
...
export function generateNautilusStrategy(options: StrategyExportOptions): ExportResultItem {
  ...
  switch (normType) {
    case "supertrend": {
      ...
      onStartLogic = `        self.atr = self.indicators.average_true_range(self.atr_period)\n        self.register_indicator_for_bars(self.bar_type, self.atr)\n        self.subscribe_bars(self.bar_type)`;
    ...
    case "ema_crossover": {
      ...
      onStartLogic = `        self.fast_ema = self.indicators.exponential_moving_average(self.fast_period, price_type=PriceType.LAST)\n        self.slow_ema = self.indicators.exponential_moving_average(self.slow_period, price_type=PriceType.LAST)\n        self.register_indicator_for_bars(self.bar_type, self.fast_ema)\n        self.register_indicator_for_bars(self.bar_type, self.slow_ema)\n        self.subscribe_bars(self.bar_type)`;
    ...
    case "rsi_oversold": {
      ...
      onStartLogic = `        self.rsi = self.indicators.relative_strength_index(self.period, price_type=PriceType.LAST)\n        self.register_indicator_for_bars(self.bar_type, self.rsi)\n        self.subscribe_bars(self.bar_type)`;
    ...
    case "keltner": {
      ...
      initMembers = `        self.period = config.period\n        self.multiplier = config.multiplier\n        self.ema = None\n        self.atr = None`;
      onStartLogic = `        self.ema = self.indicators.exponential_moving_average(self.period, price_type=PriceType.LAST)\n        self.atr = self.indicators.average_true_range(self.period)\n        self.register_indicator_for_bars(self.bar_type, self.ema)\n        self.register_indicator_for_bars(self.bar_type, self.atr)\n        self.subscribe_bars(self.bar_type)`;
    ...
    case "macd_cross": {
      ...
      onStartLogic = `        self.macd = self.indicators.moving_average_convergence_divergence(self.fast_period, self.slow_period, self.signal_period, price_type=PriceType.LAST)\n        self.register_indicator_for_bars(self.bar_type, self.macd)\n        self.subscribe_bars(self.bar_type)`;
    ...
    case "sma_golden": {
      ...
      configFields = `    fast_period: int = ${fastP}\n    slow_period: int = ${slowP}`;
      initMembers = `        self.fast_period = config.fast_period\n        self.slow_period = config.slow_period\n        self.fast_sma = None\n        self.slow_sma = None`;
      onStartLogic = `        self.fast_sma = self.indicators.simple_moving_average(self.fast_period, price_type=PriceType.LAST)\n        self.slow_sma = self.indicators.simple_moving_average(self.slow_period, price_type=PriceType.LAST)\n        self.register_indicator_for_bars(self.bar_type, self.fast_sma)\n        self.register_indicator_for_bars(self.bar_type, self.slow_sma)\n        self.subscribe_bars(self.bar_type)`;
